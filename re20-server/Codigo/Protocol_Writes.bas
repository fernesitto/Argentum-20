...
Public Function PrepareMessageMeditateToggle(ByVal charindex As Integer, _
       ...
       ' If UserList(charindex).flags.Meditando = False Then
        '    Call SendData(SendTarget.ToPCArea, charindex, PrepareMessageParticleFX(UserList(charindex).Char.charindex, UserList(charindex).Char.charindex, 15, -1, True))
        'End If
        '</EhHeader>
        If Not UserList(charindex).flags.Meditando Then UserList(charindex).Char.ParticulaFx = 0
100     Call Writer.WriteInt16(ServerPacketID.MeditateToggle)
102     Call Writer.WriteInt16(charindex)
104     Call Writer.WriteInt16(Particula)
105     Call Writer.WriteInt8(x)
