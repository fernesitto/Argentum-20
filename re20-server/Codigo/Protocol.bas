...
104         If Item = 0 Then Exit Sub
            ...
            If Cantidad > 0 Then
                UserList(UserIndex).Counters.Trabajando = UserList(UserIndex).Counters.Trabajando + 1
                UserList(UserIndex).Trabajo.TargetSkill = e_Skill.Alquimia
                UserList(UserIndex).Trabajo.Cantidad = Cantidad
                UserList(UserIndex).Trabajo.Item = Item
                Call WriteMacroTrabajoToggle(UserIndex, True)
            Else
...
122                         '.Char.FX = e_Meditaciones.MeditarInicial
                            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageParticleFX(UserList(UserIndex).Char.charindex, .Char.ParticulaFx, -1, False))
...
146         'Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessageMeditateToggle(.Char.charindex, Particula, .pos.x, .pos.y))
...
106         Vida = 18 + ModRaza(.raza).Constitucion + Promedio * (.Stats.ELV - 1)
            Else
            Vida = 18 + ModRaza(.raza).Constitucion + Promedio * (49)
            End If
