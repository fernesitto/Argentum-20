...
428                             If Abs(.pos.x - .Trabajo.Target_X) + Abs(.pos.y - .Trabajo.Target_Y) > 1 Then
...
434                             If .pos.x = .Trabajo.Target_X And .pos.y = .Trabajo.Target_Y Then
370                                 Call WriteConsoleMsg(UserIndex, "No podés quitar raices desde allí.", e_FontTypeNames.FONTTYPE_INFO)
...
382                                 Call WriteConsoleMsg(UserIndex, "El árbol ya no te puede entregar mas raices.", e_FontTypeNames.FONTTYPE_INFO)
...
440
                                ...
                                If ObjData(DummyInt).OBJType = e_OBJType.otArboles Then
390                                 Call DoRaices(UserIndex, .Trabajo.Target_X, .Trabajo.Target_Y, ObjData(.invent.HerramientaEqpObjIndex).Dorada = 1)
...
398                                 Call WriteMacroTrabajoToggle(UserIndex, False)
                    ...
                    If UserList(UserIndex).clase = e_Class.Trabajador Then
                        cantidad_maxima = UserList(UserIndex).Stats.UserSkills(e_Skill.Alquimia)
                    ...
                    Call AlquimistaConstruirItem(UserIndex, UserList(UserIndex).Trabajo.Item, UserList(UserIndex).Trabajo.Cantidad, cantidad_maxima)
                End Select
...
362                             If Abs(.pos.x - .Trabajo.Target_X) + Abs(.pos.y - .Trabajo.Target_Y) > 1 Then
                             ...
                             If .pos.x = .Trabajo.Target_X And .pos.y = .Trabajo.Target_Y Then
                                Call WriteConsoleMsg(UserIndex, "No podés talar desde allí.", e_FontTypeNames.FONTTYPE_INFO)
                                Call WriteWorkRequestTarget(UserIndex, 0)
                                    Exit Sub
                            ...
                            If ObjData(DummyInt).Elfico <> ObjData(.invent.HerramientaEqpObjIndex).Elfico Then
                                 Call WriteConsoleMsg(UserIndex, "Sólo puedes talar árboles elficos con un hacha élfica.", e_FontTypeNames.FONTTYPE_INFO)
                                 Call WriteWorkRequestTarget(UserIndex, 0)
                                    Exit Sub

                                End If
                             If ObjData(DummyInt).Pino <> ObjData(.invent.HerramientaEqpObjIndex).Pino Then
                                ...
                                End If
                             If MapData(.pos.map, .Trabajo.Target_X, .Trabajo.Target_Y).ObjInfo.amount <= 0 Then
                                 Call WriteConsoleMsg(UserIndex, "El árbol ya no te puede entregar mas leña.", e_FontTypeNames.FONTTYPE_INFO)
                                 Call WriteWorkRequestTarget(UserIndex, 0)
                                Call WriteMacroTrabajoToggle(UserIndex, False)
                                    Exit Sub
                                ...
                                '¡Hay un arbol donde clickeo?
                             If ObjData(DummyInt).OBJType = e_OBJType.otArboles Then
                                 Call DoTalar(UserIndex, .Trabajo.Target_X, .Trabajo.Target_Y, ObjData(.invent.HerramientaEqpObjIndex).Dorada = 1)

                                End If

                            Else
                             Call WriteConsoleMsg(UserIndex, "No hay ningún árbol ahí.", e_FontTypeNames.FONTTYPE_INFO)
                            Call WriteWorkRequestTarget(UserIndex, 0)
...
Sub AlquimistaQuitarMateriales(ByVal UserIndex As Integer, ByVal ItemIndex As Integer, ByVal Cantidad As Integer)
        
        On Error GoTo AlquimistaQuitarMateriales_Err
        

100     If ObjData(ItemIndex).Raices > 0 Then Call QuitarObjetos(Raices, Cantidad, UserIndex)
...
Function AlquimistaTieneMateriales(ByVal UserIndex As Integer, ByVal ItemIndex As Integer, ByVal Cantidad As Long) As Boolean
        
        On Error GoTo AlquimistaTieneMateriales_Err
        
    
100     If ObjData(ItemIndex).Raices > 0 Then
102         If Not TieneObjetos(Raices, ObjData(ItemIndex).Raices * Cantidad, UserIndex) Then
...
Public Sub AlquimistaConstruirItem(ByVal UserIndex As Integer, ByVal ItemIndex As Integer, ByVal Cantidad As Long, ByVal cantidad_maxima As Integer)
...
104     If AlquimistaTieneMateriales(UserIndex, ItemIndex, cantidad_a_construir) _
...
108         Call WriteUpdateSta(UserIndex)
            ...
            If ObjData(ItemIndex).Raices > 0 Then
                raiz_requerida = ObjData(ItemIndex).Raices
            End If


            Call AlquimistaQuitarMateriales(UserIndex, ItemIndex, raiz_requerida * cantidad_a_construir)
...
114         MiObj.amount = cantidad_a_construir
128         MiObj.objIndex = ItemIndex
...
AlquimistaConstruirItem_Err:
     Call TraceError(Err.Number, Err.Description, "Trabajo.AlquimistaConstruirItem", Erl)
130
...
Public Sub DoRaices(ByVal UserIndex As Integer, ByVal x As Byte, ByVal y As Byte, Optional ByVal ObjetoDorado As Boolean)
...
120         If res < 7 Then
                ...
                ' Else
122             MiObj.amount = GetExtractResourceForLevel(.Stats.ELV)
                ' End If
...
146             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessagePlayWave(SND_TIJERAS, .pos.x, .pos.y))
            Else
148             Call SendData(SendTarget.ToPCAliveArea, UserIndex, PrepareMessagePlayWave(SND_TIJERAS, .pos.x, .pos.y))
...
Public Function GetExtractResourceForLevel(ByVal level As Integer) As Integer
    ...
    GetExtractResourceForLevel = 7 + RandomNumber(lower, upper)
End Function
